...
/* memo
   increment change i++ -> ++i
   reduce calc for initialize (put some calc outside the loop)
   track progress is almost nothing
*/
...
//#define NPES            80        // number of processors for cpu
#define NPES            4        // number of processors for gpu
...
int main(int argc, char *argv[]) {
    ...
    MPI_Request request_down, request_up;
...
#else // non-bloocking communication
        // send bottom real row down
        if(my_PE_num != npes-1){             //unless we are bottom PE
	  MPI_Isend(&Temperature[ROWS][1], COLUMNS, MPI_DOUBLE, my_PE_num+1, DOWN, MPI_COMM_WORLD, &request_down);
        }

        // receive the bottom row from above into our top ghost row
        if(my_PE_num != 0){                  //unless we are top PE
	  MPI_Irecv(&Temperature_last[0][1], COLUMNS, MPI_DOUBLE, my_PE_num-1, DOWN, MPI_COMM_WORLD, &request_down);
        }

        // send top real row up
	if(my_PE_num != 0){                    //unless we are top PE
	  MPI_Isend(&Temperature[1][1], COLUMNS, MPI_DOUBLE, my_PE_num-1, UP, MPI_COMM_WORLD, &request_up);
	}

        // receive the top row from below into our bottom ghost row
	if(my_PE_num != npes-1){             //unless we are bottom PE
	  MPI_Irecv(&Temperature_last[ROWS+1][1], COLUMNS, MPI_DOUBLE, my_PE_num+1, UP, MPI_COMM_WORLD, &request_up);
        }
...
void initialize(int npes, int my_PE_num){
    ...
    grad_row = (tMax-tMin)/ROWS;
    grad_col = 100.0/COLUMNS;

    // Left and right boundaries
    for (i = 0; i <= ROWS+1; ++i) {
      Temperature_last[i][0] = 0.0;
      //      Temperature_last[i][COLUMNS+1] = tMin + ((tMax-tMin)/ROWS)*i;
      Temperature_last[i][COLUMNS+1] = tMin + grad_row*i;
    }
    ...
    if (my_PE_num == npes-1)
      for (j=0; j<=COLUMNS+1; ++j)
	Temperature_last[ROWS+1][j] = grad_col * j;
